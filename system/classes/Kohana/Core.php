...
class Kohana_Core {
	...
	public static function auto_load_aggressive($class, $directory = 'classes')
	{
		// Transform the class name according to PSR-0
		$class     = ltrim($class, '\\');
		$file      = '';
		...
		if ($path = Kohana::find_file_aggressive($directory, $file))
		{
			// Load the class file
			require $path;
	 ...
	 */
	public static function find_file($dir, $file, $ext = NULL, $array = FALSE, $aggressive = FALSE)
	{
		if ($ext === NULL)
		{
		...
		if ($array OR $dir === 'config' OR $dir === 'i18n' OR $dir === 'messages')
		{
			...
			{
				if ($p = self::file_exists($dir.$path, $aggressive, $dir))
				{
	...
	public static function find_file_aggressive($dir, $file, $ext = NULL, $array = FALSE)
	{
	 ...
	 */
	public static function file_exists($file, $aggressive = FALSE, $root = NULL)
	{
		if (is_file($file))
		{
			return $file;
		...
		foreach ($components as $component)
		{
			...
			if ($reconstruction !== '' and !is_dir($reconstruction))
			{
				return FALSE;
			}

			$ls = scandir($reconstruction !== '' ? $reconstruction : DIRECTORY_SEPARATOR);
			if ($ls === FALSE)
			{
				return FALSE;
			}

			$index = array_search(strtolower($component), array_map('strtolower', $ls));
			if ($index === false)
			{
				return FALSE;
			}

			$reconstruction .= DIRECTORY_SEPARATOR . $ls[$index];
		}
